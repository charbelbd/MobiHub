...
export default function POS({ refresh }) {
  ...
  </section><aside className="orderBox"><h2>POS Order</h2><label>Client name (optional)<input value={clientName} onChange={e=>setClientName(e.target.value)} placeholder="Client name" /></label><label>Quantity before selecting<input type="number" min="1" value={qty} onChange={e=>setQty(e.target.value)} /></label><div className="cartList">{cart.map(i => <div className="cartItem" key={i.product_id}><div><b>{i.name}</b><span>{i.quantity} × {money(i.price)} profit {money(i.profit_price)} each</span></div><strong>{money(i.quantity*i.price)}</strong><button onClick={()=>setCart(cart.filter(x=>x.product_id!==i.product_id))}><Trash2 size={16}/></button></div>)}</div><div className="discountRow"><select value={discountType} onChange={e=>setDiscountType(e.target.value)}><option>%</option><option>$</option></select><input type="number" value={discount} onChange={e=>setDiscount(e.target.value)} placeholder="Discount"/></div><div className="totals"><span>Subtotal {money(subtotal)}</span><span>Profit {money(profitSubtotal)}</span><span>Discount {discountType === '%' ? `${discount || 0}%` : money(discount || 0)}</span><b>Total {money(total)}</b></div><button className="secondary" onClick={()=>submit(false)}>Pay Later</button><button className="primary" onClick={()=>submit(true)}>Submit Order</button></aside></div>;
}
